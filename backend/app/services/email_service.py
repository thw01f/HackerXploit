...
def send_registration_verify_email(user_id, verify_code):
    ...
    with app.app_context():
        ...
        body = f"<h2>Welcome to HackerXploit, {user.username}!</h2><p>Your verification code is: <strong>{verify_code}</strong></p>"
...
def send_account_status_email(user_id, status):
    ...
    with app.app_context():
        ...
        body = f"<h2>Account Status Update</h2><p>Hello {user.username}, your club membership status has been updated to: <strong>{status}</strong>.</p>"
        success = send_smtp_email(user.email, subject, body)
...
def _send_offline_inbox_email_impl(user_id, subject_text, snippet):
    ...
    body = f"<h2>New Message in HackerXploit Inbox</h2><p>Hello {user.username}, you received a new message:</p><blockquote>{snippet}</blockquote><p><a href='http://club.hackerxploit.org/inbox'>Log in to view complete message</a></p>"
    success = send_smtp_email(user.email, subject, body)
