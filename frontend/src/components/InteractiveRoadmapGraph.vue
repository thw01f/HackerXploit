  ...
  <div class="relative w-full h-full bg-[#0b0e14] text-slate-100 font-mono overflow-hidden select-none">
    ...
    <div class="absolute top-20 left-6 z-20 bg-[#161b22]/95 backdrop-blur-md p-4 rounded-xl border border-[#21262d] shadow-2xl text-xs space-y-3 max-w-xs">
      <div class="flex items-center justify-between border-b border-[#21262d] pb-2">
        <span class="font-extrabold text-white uppercase text-[11px] tracking-wider">// ROADMAP</span>
    ...
    <div
      v-if="selectedNode"
      class="fixed inset-y-0 right-0 z-50 w-full sm:w-[520px] bg-[#161b22] border-l border-[#21262d] shadow-2xl flex flex-col transform transition-transform duration-300 ease-out"
    ...
    >
      <div class="p-6 border-b border-[#21262d] flex items-center justify-between bg-[#0b0e14]/50">
        <div>
          <span class="text-xs text-[#00f0ff] font-bold uppercase tracking-wider block mb-1">
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        <div class="bg-[#0b0e14] p-4 rounded-xl border border-[#21262d] flex items-center justify-between">
          ...
          <button
            ...
            class="px-5 py-2.5 text-sm font-extrabold rounded-lg border transition-all flex items-center space-x-2 shadow-lg"
        ...
        <div class="prose prose-invert max-w-none text-sm leading-relaxed space-y-4">
        ...
        <div v-if="selectedNode.resources && selectedNode.resources.length > 0" class="pt-4 border-t border-[#21262d] space-y-4">
          <h3 class="text-sm font-extrabold text-[#00f0ff] uppercase tracking-wider">// LEARNING RESOURCES</h3>
          ...
          <div class="space-y-2">
            <a
              ...
              class="p-3.5 bg-[#0b0e14] hover:bg-[#21262d] rounded-xl border border-[#21262d] hover:border-[#00f0ff]/50 transition-all flex items-center justify-between group"
            >
              <div class="flex items-center space-x-3">
                ...
                <div>
                  <h4 class="text-sm font-bold text-slate-200 group-hover:text-[#00f0ff] transition-colors">{{ res.title }}</h4>
                  <span class="text-xs text-slate-500 truncate block max-w-[240px]">{{ res.url }}</span>
                </div>
              </div>
...
// Re-fetch when the parent swaps roadmapSlug (e.g. the Academy roadmap
// picker dropdown) - without this the canvas kept showing whatever roadmap
// was active on mount, silently ignoring dropdown changes.
watch(() => props.roadmapSlug, () => {
